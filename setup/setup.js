...
years.value=localStorage.getItem('yearsExperience')||'';role.value=localStorage.getItem('targetRole')||'';answerProvider.value=localStorage.getItem('answerProvider')||'openai';
async function loadPreviousSetup(){
  ...
  if(previousSetup.answerProvider){const saved=String(previousSetup.answerProvider);answerProvider.value=['openai','terra','luna','gpt4o','gpt4omini','cerebras'].includes(saved)?saved:'openai';}
...
prepareBtn.onclick=async()=>{showError('');const resumeFile=resumeInput.files?.[0],jdFile=jdInput.files?.[0],pastedJd=jdText.value.trim();
  ...
  try{
    ...
    years.value=Number.isFinite(Number(inferredYears))?String(inferredYears):'';
    role.value=inferredRole;
    if(years.value)localStorage.setItem('yearsExperience',years.value);else localStorage.removeItem('yearsExperience');
    if(role.value)localStorage.setItem('targetRole',role.value);else localStorage.removeItem('targetRole');
    localStorage.setItem('answerProvider',answerProvider.value);
    progressTitle.textContent='Interview context ready';
    const inferred=[years.value?`${years.value} years`:'',role.value].filter(Boolean).join(' · ');
    progressText.textContent=`${result.chunkCount||0} searchable chunks prepared${inferred?` · ${inferred}`:''}. Opening live overlay…`;
    await new Promise(r=>setTimeout(r,250));
    const opened=await window.electronAPI.openOverlayAfterSetup();
    if(!opened?.success)throw new Error(opened?.error||'Account validation failed before opening the listening screen.');
