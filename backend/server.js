...
function answerTokenBudget(question, hasImage=false,responseType='') {
  ...
  // Give normal spoken answers enough room for a direct answer plus practical explanation.
  // This does not add another model call and therefore preserves the existing streaming/latency path.
  return 420;
}
...
3. Prefer a natural interview answer rather than a keyword summary. The opening sentence should answer the question immediately; the following sentences should expand the key implementation steps in plain spoken language. Short paragraphs are fine. Use bullets only when the interviewer asks for a list/count or when 3-4 distinct troubleshooting/implementation steps are materially clearer as separate points. Never pad with generic theory.
