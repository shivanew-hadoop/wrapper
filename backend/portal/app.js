...
function topperPdfName(ms){const parts=new Intl.DateTimeFormat('en-US',{timeZone:'Asia/Kolkata',month:'short',day:'numeric',hour:'numeric',minute:'2-digit',hour12:true}).formatToParts(new Date(Number(ms)||Date.now()));const get=t=>parts.find(p=>p.type===t)?.value||'';const d=Number(get('day'));const m=d%100;const suffix=(m>=11&&m<=13)?'th':({1:'st',2:'nd',3:'rd'}[d%10]||'th');return `Topper_${get('month')}${d}${suffix}_${get('hour')}${get('minute')}${get('dayPeriod').toUpperCase()}_IST.pdf`;}
...
async function auth(kind){try{const d=await api(`/api/auth/${kind}`,{method:'POST',body:JSON.stringify({name:$('name').value,email:$('email').value,password:$('password').value})});token=d.token;localStorage.setItem('topperToken',token);history.replaceState(null,'',location.pathname);await load();$('account')?.scrollIntoView({behavior:'smooth',block:'start'})}catch(e){message(e.message,true)}}
