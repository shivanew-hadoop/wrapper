...
function renderInterviewHistory(){
  ...
  target.innerHTML=interviewSessions.map((session,index)=>`<article class="savedSession"><div class="savedSessionSummary"><div><strong>${esc(sessionWhen(session.endedAt))}</strong><small>${Number(session.turnCount)||0} question${Number(session.turnCount)===1?'':'s'} · completed session</small></div><div class="savedSessionActions"><button class="historyToggle viewTranscript" data-index="${index}">View</button><button class="historyToggle downloadTranscript" data-id="${esc(session.id)}">Download PDF</button></div></div><div class="savedTurns hidden" id="savedTurns-${index}">${(session.turns||[]).map((turn,turnIndex)=>`<div class="savedTurn"><time>${esc(turnTime(turn.askedAt))}</time><strong>Q${turnIndex+1}. ${esc(turn.question)}</strong><p>${esc(turn.answer)}</p><div class="savedDots">· · · · · · · · · · · ·</div></div>`).join('')}</div></article>`).join('');
