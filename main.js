...
function saveSetupDefaults(payload) {
  try {
    ...
    const compact={
      ...
      answerProvider:['openai','terra','luna','gpt4o','gpt4omini','gemini','cerebras'].includes(String(payload?.answerProvider||'openai'))?String(payload?.answerProvider||'openai'):'openai',
      savedAt:Date.now()
    };
