...
3. Create a Razorpay test-mode account. Put the test key ID and secret in `.env`. Create a different random `RAZORPAY_WEBHOOK_SECRET`.
...
7. Use Razorpay test checkout to buy the ₹599 plan. The browser verifies the checkout signature immediately; the signed webhook provides idempotent server-to-server recovery. Confirm exactly 3,600 seconds are credited once, even if the webhook is replayed.
...
4. Set `NODE_ENV=production`, `LEGACY_LICENSE_FALLBACK=false`, a permanent `JWT_SECRET`, live OpenAI/Deepgram keys, Razorpay test keys initially, and the real admin email. Restrict inbound backend traffic to ports 443/80; do not expose the database volume.
5. Configure Razorpay test webhook as `https://api.yourdomain.com/api/payments/webhook`, select `payment.captured`, and use the exact webhook secret from the server. Razorpay requires a public URL; localhost is not a production webhook target.
...
11. Complete Razorpay KYC/activation, publish Terms, Privacy, Refund/Cancellation and support/contact pages, confirm tax/GST obligations with an Indian accountant, then switch both checkout keys and webhook to live mode. Run one real ₹599 purchase and refund/reconciliation test before public launch.
...
5. Add a scheduled reconciler that compares captured Razorpay payments with paid orders and alerts on mismatches. Add dispute/refund handling that writes compensating ledger entries instead of deleting history.
